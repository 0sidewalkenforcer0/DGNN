...
from models.models_statements import StarE_Transformer
...
DEFAULT_CONFIG = {
    ...
    'MAX_QPAIRS': 40,
    'MODEL_NAME': 'stare_transformer', #StarE_trans_baseline, stare_transformer
    'CORRUPTION_POSITIONS': [0, 2],
    # 'TEST_MODEL_PATH': './models/yago1830/stare_transformer/0', #None
...
if __name__ == "__main__":
    ...
    if config['USE_TEST']:
        train_data_gcn = DataManager.get_alternative_graph_repr(train_data + valid_data, static_data)
    else:
        train_data_gcn = DataManager.get_alternative_graph_repr(train_data, static_data)
    """
        Make the model.
    """
    ...
    if config['TEST_MODEL_PATH'] is not None:
        ...
        model_state_dict = torch.load(config['TEST_MODEL_PATH']+'/model.torch')
