...
class StarEEncoder(StarEBase):
    ...
    def forward_base(self, sub, rel, time, drop1, drop2,
        ...
        # Time-Encoding
        sub_emb = self.get_ent_time_emb(sub_emb, time[:, np.newaxis])
        rel_emb = torch.index_select(r, 0, rel)

        if  self.embed_statics:
            all_entity = self.static_emb(self.static_all, x, r)
            sub_static = torch.index_select(self.static_all, 0, sub)
            sub_static_emb = torch.index_select(all_entity, 0, sub)
            ...
            if self.embed_qualifiers:
                ...
                if not return_mask:
                    return sub_emb, rel_emb, qual_emb, sub_static_emb, all_entity
                else:
                    ...
                    qual_mask = torch.zeros((sub.shape[0], quals.shape[1] + 2)).bool().to(self.device)
                    # and put True where qual entities and relations are actually padding index 0
                    qual_mask[:, 2:] = quals == 0
                    static_mask = torch.zeros((sub_static.shape[0], sub_static.shape[1])).bool().to(self.device)
                    static_mask = sub_static == 0
                    mask = torch.cat((qual_mask, static_mask), dim=1)
                    return sub_emb, rel_emb, qual_emb, sub_static_emb, all_entity, mask
            else:
                if not return_mask:
                    return sub_emb, rel_emb, None, sub_static_emb, all_entity, None
                else:
                    static_mask = torch.zeros((sub_static.shape[0], sub_static.shape[1] + 2)).bool().to(self.device)
                    static_mask[:, 2:] = sub_static == 0
                    return sub_emb, rel_emb, None, sub_static_emb, all_entity, static_mask
            ...
            qual_emb = torch.cat((qual_rel_emb, qual_obj_emb), 2).view(-1, 2 * qual_rel_emb.shape[1],
                                                                       qual_rel_emb.shape[2])
            if not return_mask:
                return sub_emb, rel_emb, qual_emb, None, x[:self.raw_num_ent], None
            else:
                qual_mask = torch.zeros((sub.shape[0], quals.shape[1] + 2)).bool().to(self.device)
                qual_mask[:, 2:] = quals == 0
                return sub_emb, rel_emb, qual_emb, None, x[:self.raw_num_ent], qual_mask
        else:
           return sub_emb, rel_emb, None, None, x[:self.raw_num_ent], None
