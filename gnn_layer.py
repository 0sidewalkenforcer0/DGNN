...
class StarEConvLayer(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, edge_time, rel_embed,
        ...
        if self.p['SAMPLER_W_STATICS']:
            ...
            sta_out = self.propagate(static_index, x=x, edge_type=static_type, time=None,
                                     ...
                                     ent_embed=x,
                                     qualifier_ent=static_index[1],
                                     qualifier_rel=static_type,
                                     qual_index=static_index[0])
    ...
    def static_aggregate(self, qualifier_emb, qual_index=None):

        if self.p['STAREARGS']['QUAL_AGGREGATE'] == 'sum':
            qualifier_emb = torch.einsum('ij,jk -> ik',
                                         self.coalesce_quals(qualifier_emb, qual_index, qualifier_emb.shape[0]),
                                         self.w_q)
            return qualifier_emb      # [N_EDGES / 2 x EMB_DIM]
        elif self.p['STAREARGS']['QUAL_AGGREGATE'] == 'concat':
    ...
    def update_with_static(self, ent_embed, rel_embed,
                                      static_sub, static_rel, static_obj):

        # Step 1: embedding
        static_emb_rel = rel_embed[static_rel]
        static_emb_obj = ent_embed[static_obj]

        # rel_part_emb = rel_embed[edge_type]

        # Step 2: pass it through qual_transform
        static_emb = self.qual_transform(qualifier_ent=static_emb_obj,
                                            qualifier_rel=static_emb_rel)

        # Pass it through a aggregate layer
        return self.static_aggregate(static_emb, qual_index=static_sub)
        # version 2: every static has one weights
        # return torch.einsum('ij,jk->ik', qualifier_emb, self.w_q)
    ...
    def message(self, x_j, x_i, edge_type, time, rel_embed, edge_norm, mode, ent_embed=None, qualifier_ent=None,
        ...
        if mode != 'static':
            ...
            out = self.update_with_static(ent_embed, rel_embed, qual_index,
                                             qualifier_rel, qualifier_ent)
        else:
